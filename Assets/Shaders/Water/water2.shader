	...
	SubShader {
		...
		CGPROGRAM
			...
			void disp (inout appdata v) {
				fixed waveTex1 = tex2Dlod(_WaveTex1H, float4(v.texcoord.xy * _WaveTiling.x, 0, 0)).a * _Displace.x;
				fixed waveTex2 = tex2Dlod(_WaveTex2H, float4(v.texcoord.xy * _WaveTiling.y, 0, 0)).a * _Displace.y;
			...
			void surf (Input IN, inout SurfaceOutputStandard o) {
				fixed4 waveTex1 = tex2D(_WaveTex1, IN.uv_WaveTex1 * _WaveTiling.x);
				fixed4 waveTex2 = tex2D(_WaveTex2, IN.uv_WaveTex1 * _WaveTiling.y);
